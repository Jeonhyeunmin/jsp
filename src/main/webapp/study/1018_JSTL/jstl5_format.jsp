...
<head>
  ...
  <title>jstl5_foramt.jsp</title>
	...
	<div class="container">
  	...
  	<div>형식문자열(천단위 쉼표, 화폐단위, 백분율, 날짜형식...)</div>
  	...
  	<div>
  		...
  		<fmt:formatNumber value="${won1}"/> / <fmt:formatNumber value="${won2}"/> / <fmt:formatNumber value="7654321"/> <br>
			...
			백분율2 : <fmt:formatNumber value="${per}" type="percent" pattern="#.0%"/> <br>
			
			<b>4. 날짜</b> <br>
			오늘 날짜 1 : <%= new Date() %> <br>
			<c:set var="today" value="<%= new Date() %>"/>
			오늘 날짜 2 : <font color="red"><fmt:formatDate value="${today}" pattern="yyyy-MM-dd"/></font> <br>
			오늘 날짜 3 : <font color="red"><fmt:formatDate value="${today}" pattern="yyyy-MM"/></font> <br>
			오늘 날짜 4 : <font color="red"><fmt:formatDate value="${today}" pattern="MM-dd"/></font> <br>
			오늘 날짜 5 : <font color="red"><fmt:formatDate value="${today}" pattern="yyyy년 MM월 dd일"/></font> <br>
			...
			6. URL 이동 : location.href / response.sendRedirect ==> redirect(core에 있음) <br>
