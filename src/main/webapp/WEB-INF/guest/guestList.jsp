	...
	<div class="container">
  	...
  	<table class="table table-borderless m-0 p-0">
  		<tr>
  			...
  			<td class="text-right m-0 p-0">
  				<c:if test="${pag > 1}">
  					<a href="GuestList.gu?pag=1" title="첫페이지" style="color: black;">◀◀</a>&nbsp;
  					<a href="GuestList.gu?pag=${pag-1}" title="이전페이지" style="color: black;">◀</a>
  				</c:if>
  				${pag} / ${totPage}
  				<c:if test="${pag < totPage}">
  					<a href="GuestList.gu?pag=${pag+1}" title="다음페이지" style="color: black;">▶</a>
  					<a href="GuestList.gu?pag=${totPage}" title="다음페이지" style="color: black;">▶▶</a>
  				</c:if>
  			</td>
  		</tr>
  	</table>
  	...
  	<ul class="pagination justify-content-center">
  	  <li class="page-item">
			...
			<li class="page-item">
				<c:if test="${pag > 1}"><a class="page-link" href="GuestList.gu?pag=1">첫 페이지</a></c:if>
        <c:if test="${pag <= 1}"><span class="page-link disabled">첫페이지</span></c:if>
			...
			<li class="page-item"><c:if test="${curBlock > 0}"><a class="page-link" href="GuestList.gu?pag=${(curBlock-1)*blockSize + 1}">이전블록</a></c:if></li>
			
		  <c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize)+blockSize}" varStatus="st">
			  <c:if test="${i <= totPage && i == pag}"><li class="page-item active"><a class="page-link" href="GuestList.gu?pag=${i}"><b>${i}</b></a></li></c:if>
		  ...
		  <li class="page-item">
				...
				<c:if test="${pag < totPage}"><a class="page-link" href="GuestList.gu?pag=${totPage}">마지막 페이지</a></c:if>
      </li>
		</ul>
  	<!-- 블록페이지 끝 -->
