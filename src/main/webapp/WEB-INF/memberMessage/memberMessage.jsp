...
<head>
  ...
  <script>
    ...
    function memberDeleteCheck(idx) {
 ...
 // 메세지 내용 모달로 띄우기
    function messageContentView(senderId, content) {
    	$("#myModal2 #senderId").text(senderId);
    	$("#myModal2 #content").html(content);
    }
  </script>
    ...
    <table class="table table-hover text-center">
      ...
      <c:forEach var="vo" items="${messageVos}" varStatus="st">
        <tr>
          <td>${no}</td>
          <td>${vo.senderId}</td>
          <td><a href="#" onclick="messageContentView('${vo.senderId}','${fn:replace(vo.content,newLine,'<br/>')}')" data-toggle="modal" data-target="#myModal2" title="메세지 내용보기">${vo.content}</a></td>
          <td>${vo.msgDate}</td>
          <td><a href="javascript:memberDeleteCheck(${vo.idx})" class="btn btn-danger btn-sm">삭제</a></td>
        </tr>
...
<!-- 받은/보낸 메세지 내용을 모달로 보여주기 -->
<div class="modal" id="myModal2">
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="m-3">
        <div class="row">
          <div class="col"><h5>메세지</h5></div>
        </div>
      </div>
      <div class="modal-body">
